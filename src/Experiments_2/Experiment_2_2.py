...
parser.add_argument("--SEED_TARGETS_DIR"   , "--seed_targets_directory"     , type=str, help="SEED DATASET - Location of the File Containing the Targets in VNC-Token format.")
parser.add_argument("--SEED_CFORM_DIR"     , "--seed_canonical_forms"       , type=str, help="SEED DATASET - Location of the File Indicating the Canonical Forms of the Candidates.")
parser.add_argument("--SEED_SYN_FIX_DIR"   , "--seed_syntactical_fixedness" , type=str, help="SEED DATASET - Location of the File Indicating the Syntactical Fixedness of the Candidates.")
parser.add_argument("--SEED_LEX_FIX_DIR"   , "--seed_lexical_fixedness"     , type=str, help="SEED DATASET - Location of the File Indicating the Lexical Fixedness of the Candidates.")
parser.add_argument("--SEED_OVA_FIX_DIR"   , "--seed_overall_fixedness"     , type=str, help="SEED DATASET - Location of the File Indicating the Overall Fixedness of the Candidates.")
parser.add_argument("--SEED_VECTORS_FILE"  , "--seed_embedded_vectors_file" , type=str, help="SEED DATASET - Name of the Embeddings File.")
...
SEED_TARGETS_DIR   = "../targets/English_VNC_Cook/VNC-Tokens_cleaned"
SEED_CFORM_DIR     = "../targets/CForms.csv"
SEED_SYN_FIX_DIR   = "../targets/SynFix.csv"
SEED_LEX_FIX_DIR   = "../targets/LexFix.csv"
SEED_OVA_FIX_DIR   = "../targets/OvaFix.csv"
...
def main():
    ...
    SEED_targets = pd.read_csv(SEED_TARGETS_DIR, header=None, usecols=[0], sep=' ').values.flatten()
    SEED_indexes = np.where(SEED_targets != 'Q')

    SEED_targets_idiomatic = (SEED_targets[SEED_indexes] == 'I')
    SEED_targets_literal   = (SEED_targets[SEED_indexes] == 'L')

    SEED_features_w2v   = np.genfromtxt(W2V_DIR   + SEED_VECTORS_FILE, delimiter=',')[SEED_indexes]
    SEED_features_scbow = np.genfromtxt(SCBOW_DIR + SEED_VECTORS_FILE, delimiter=',')[SEED_indexes]
    SEED_features_skip  = np.genfromtxt(SKIP_DIR  + SEED_VECTORS_FILE, delimiter=',')[SEED_indexes]
    SEED_features_elmo  = np.genfromtxt(ELMO_DIR  + SEED_VECTORS_FILE, delimiter=',')[SEED_indexes]

    # -- ADD FAZLY's METRICS -- #
    if(USE_CFORM):
        SEED_cForms = np.genfromtxt(SEED_CFORM_DIR, delimiter=',')[SEED_indexes]
        SEED_cForms = SEED_cForms.reshape((SEED_cForms.size, 1))

        SEED_features_w2v   = np.append(SEED_features_w2v,   SEED_cForms, axis=1)
        SEED_features_scbow = np.append(SEED_features_scbow, SEED_cForms, axis=1)
        SEED_features_skip  = np.append(SEED_features_skip,  SEED_cForms, axis=1)
        SEED_features_elmo  = np.append(SEED_features_elmo,  SEED_cForms, axis=1)

    if(USE_SYN_FIX):
        SEED_synFix = np.genfromtxt(SEED_SYN_FIX_DIR, delimiter=',')[SEED_indexes]
        SEED_synFix = SEED_synFix.reshape((SEED_synFix.size, 1))

        SEED_features_w2v   = np.append(SEED_features_w2v,   SEED_synFix, axis=1)
        SEED_features_scbow = np.append(SEED_features_scbow, SEED_synFix, axis=1)
        SEED_features_skip  = np.append(SEED_features_skip,  SEED_synFix, axis=1)
        SEED_features_elmo  = np.append(SEED_features_elmo,  SEED_synFix, axis=1)

    if(USE_LEX_FIX):
        SEED_lexFix = np.genfromtxt(SEED_LEX_FIX_DIR, delimiter=',')[SEED_indexes]
        SEED_lexFix = SEED_lexFix.reshape((SEED_lexFix.size, 1))

        SEED_features_w2v   = np.append(SEED_features_w2v,   SEED_lexFix, axis=1)
        SEED_features_scbow = np.append(SEED_features_scbow, SEED_lexFix, axis=1)
        SEED_features_skip  = np.append(SEED_features_skip,  SEED_lexFix, axis=1)
        SEED_features_elmo  = np.append(SEED_features_elmo,  SEED_lexFix, axis=1)

    if(USE_OVA_FIX):
        ovaFix = np.genfromtxt(SEED_OVA_FIX_DIR, delimiter=',')[SEED_indexes]
        ovaFix = SEED_ovaFix.reshape((SEED_ovaFix.size, 1))

        SEED_features_w2v   = np.append(SEED_features_w2v,   SEED_ovaFix, axis=1)
        SEED_features_scbow = np.append(SEED_features_scbow, SEED_ovaFix, axis=1)
        SEED_features_skip  = np.append(SEED_features_skip,  SEED_ovaFix, axis=1)
        SEED_features_elmo  = np.append(SEED_features_elmo,  SEED_ovaFix, axis=1)
    ...
    w2v_X_Seed, scbow_X_Seed, skip_X_Seed, elmo_X_Seed, y_Seed = resample(SEED_features_w2v, SEED_features_scbow, SEED_features_skip, SEED_features_elmo, SEED_targets_idiomatic, n_samples=SEED_SIZE, random_state=RND_STATE, replace=False, stratify=SEED_targets_idiomatic)
